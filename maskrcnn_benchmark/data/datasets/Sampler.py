...
class Sampler(torch.utils.data.Dataset):
    ...
    def __init__(self, data_dir, split, use_difficult=False, transforms=None, external_proposal=False, old_classes=[],
        ...
        self._imgsetpath = os.path.join(self.root, "ImageSets", "H_Sampler_incremental", "%s.txt")
        ...
        if self.is_train:  # training mode
            if self.data_augmentation:
                ...
                self._load_img_from_old_cls_without_new_data()
            elif self.sampler_incremental:
                print('Sampler.'
                      'py | in training mode')
            ...
            print('Sampler.py | in test mode')
            self._load_img_from_NEW_and_OLD_cls_without_old_data()

    # def _load_img_data_augmentation(self):
    ...
    def _normally_load_defect(self):
        ...
        print("Sampler.py | normally_load_defect | load data")
        with open(self._imgsetpath % self.image_set) as f:
            self.ids = f.readlines()
        self.ids = [x.strip("\n") for x in self.ids]
    ...
    def _load_img_from_NEW_and_OLD_cls_without_old_data(self):
        ...
        for w in range(len(total_classes)):
            ...
            print('Sampler.py | load_img_from_NEW_cls_without_old_data | number of images in {0}_{1} set: {2}'.format(
        ...
        print(
            'Sampler.py | load_img_from_NEW_and_OLD_cls_without_old_data | total used number of images in {0}: {1}'.format(
    ...
    def _load_img_from_NEW_cls_without_old_data(self):
        ...
        for incremental in self.new_classes:  # read corresponding class images from the data set
            ...
            # list70 = random.sample(self.ids, int(0.7 * len(self.ids)))
            # with open(self._imgsetpath % "{0}".format("train70"),'w') as f:
            #     for index in list70:
            #         f.write(index)
            #         f.write("\n")
            #     f.close()
            ...
            print('Sampler.py | load_img_from_NEW_cls_without_old_data | number of images in {0}_{1} set: {2}'.format(incremental, self.image_set, len(img_ids_per_category)))
        ...
        # store image ids and class ids
        print('Sampler.py | load_img_from_NEW_cls_without_old_data | total used number of images in {0}: {1}'.format(
            self.image_set, len(self.final_ids)))
    ...
    def _load_img_from_old_cls_without_new_data(self):
        ...
        for incremental in self.new_classes:
            ...
            print('Sampler.py | load_img_from_old_cls_without_new_data | number of images in {0}_{1} set: {2}'.format(incremental, self.image_set, len(img_ids_per_category)))
        ...
        # store image ids and class ids
        print('Sampler.py | load_img_from_old_cls_without_new_data | total used number of images in {0}: {1}'.format(
    ...
    def __getitem__(self, index):
        # index = 379
        if self.is_train and self.data_augmentation:#self.image_set =="train":
            # old_img =

            img_id = self.final_ids[index]
            img, target_list = self.mosaic(img_id,self._imgsetpath% "{0}".format(self.image_set))
            img = Image.fromarray(img)

            target = BoxList(target_list["boxes"], (target_list["im_info"][0], target_list["im_info"][1]), mode="xyxy")
            target.add_field("labels", target_list["labels"])
            target.add_field("difficult", target_list["difficult"])
            proposal = None
    ...
    def map_class_id_to_class_name(self, class_id):
        return Sampler.CLASSES[class_id]
