...
def calculate_feature_distillation_loss(source_features, target_features, loss=None,num =None):  # pixel-wise
    ...
    if num_source_features == num_target_features:
        for i in range(num_source_features):
            ...
            if loss == 'l2':
                ...
                if num is not None:
                    final_feature_distillation_loss.append(torch.sum(feature_distillation_loss) / num)
                else:
...
def calculate_roi_distillation_loss(soften_results, target_results, cls_preprocess=None, cls_loss=None, bbs_loss=None, temperature=1):
    ...
    if cls_preprocess == 'sigmoid':
        ...
        modified_target_scores = F.log_softmax(torch.cat((target_scores[:, :1],target_scores[:, tot_classes-num_of_distillation_categories+1:]),dim=1), dim=1)
    ...
    if cls_loss == 'cross-entropy':
        modified_soften_boxes = soften_bboxes[:, 1:, :]
        modified_target_bboxes = target_bboxes[:, tot_classes-num_of_distillation_categories+1:, :]
    else:
