...
def align_and_update_state_dicts(model_state_dict, load_CTN,loaded_state_dict):
    """
    ...
    for idx_new, idx_old in enumerate(idxs.tolist()):
        ...
        if model_state_dict[key].size() == loaded_state_dict[key_old].size():
            ...
            if load_CTN:
                ...
                if "predictor.bbox_pred" in key:
                    # old_num = loaded_size[0]/4
                    # new_num = new_size[0]/4
                    model_state_dict[key][:4] = loaded_state_dict[key_old][:4]
                    model_state_dict[key][new_size[0]-loaded_size[0]+4:] = loaded_state_dict[key_old][4:]

                elif"predictor.cls_score" in key:
                    model_state_dict[key][:1] = loaded_state_dict[key_old][:1]
                    model_state_dict[key][new_size[0]-loaded_size[0]+1:] = loaded_state_dict[key_old][1:]
                else:
                    raise Exception("模型加载出问题")
            else:
                try:
                    model_state_dict[key][:loaded_size[0]] = loaded_state_dict[key_old]
                except:
                    model_state_dict[key][:] = loaded_state_dict[key_old][:new_size[0]]
...
def load_state_dict(model, loaded_state_dict,load_CTN):
    ...
    align_and_update_state_dicts(model_state_dict, load_CTN,loaded_state_dict)
