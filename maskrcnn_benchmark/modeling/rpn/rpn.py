...
class RPNModule(torch.nn.Module):
    ...
    def forward(self, images, features, targets=None, rpn_output_source=None):
        ...
        """"""objectness的size为batch×anchor_num×50×50,表示特征图上每一个点对应15个anchor的得分，50×50为特征图维度,
            rpn_box_regression的size为batch×（anchor*4）_num×50×50,意思类似
        """
        objectness, rpn_box_regression = self.head(features)
        anchors = self.anchor_generator(images, features)#list长度等于batch_size,其中tensor表示每个anchor的xyxy坐标
    ...
    def _forward_test(self, anchors, objectness, rpn_box_regression):
        boxes = self.box_selector_test(anchors, objectness, rpn_box_regression)##batch中每张图选前1000个proposal
        if self.cfg.MODEL.RPN_ONLY:
